...
## Provisional academic units

The academic-unit select currently uses a mock/provisional controlled list in `mock-academic-units.js`. Both the browser validation (`main.js`) and the server validation (`api/submit.js`) read the same list so they stay aligned until the final list is available.

Replace `MOCK_ACADEMIC_UNITS` with the definitive values before production intake.

## Local run

You can open `index.html` directly in the browser for a static UI preview. The page uses relative asset paths, so CSS, JavaScript, the mock academic-unit list, and `assets/splash.jpg` load from the project folder under `file://`. If `assets/splash.jpg` is missing, the splash still advances to the form after about 2.4 seconds.
